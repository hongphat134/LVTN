...
<section class="site-section services-section bg-light block__62849" id="next-section">
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
        <h3><span class="icon-group_add mr-2"></span>Danh sách ứng viên phù hợp <span class="icon-group_add"></span></h3>
        @if(session('success'))
        ...
        @endif
       	...
       	<h3>Vị trí "{{$job->nganh}}"</h3>
        @if($seeker_list->count() != 0)
        <form action="{{url('/nhatuyendung/nhieu-ngo-y')}}">
        <table class="table table-hover table-responsive">
          ...
          <tbody>
          @foreach($seeker_list as $key => $profile)
          <tr>
            <td><a href="{{url('/nhatuyendung/thong-tin-ho-so',$profile->id)}}">{{$profile->hoten}}</a></td>
            <td>{{$profile->nganh}}</td>
    ...
    <a class="dropdown-item" target="_blank" href="{{url('nhatuyendung/ung-tuyen/pdf',[$profile->idUser,$profile->idTTD])}}">Xuất PDF</a>   
        ...
        <button type="button" id="recBtn" class="btn btn-danger" data-toggle="modal" data-target="">Ngỏ ý</button>
        @else
       	Không tìm thấy ứng viên nào phù hợp cả! Hãy kiên nhẫn chờ đợi bạn nhé!
        @endif 
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
        	...
        	<input type="hidden" name="vitri" value="{{$job->nganh}}">
          <label for="comment">Chúng tôi sẽ đại diện cho bạn gửi lời nhắn đến người tìm việc, Với nội dung là:</label>          
          <p>
          	Nhà tuyển dụng "Tên nhà tuyển dụng" ngỏ ý đến hồ sơ đăng công khai của bạn phù hợp với vị trí tuyển dụng "vị trí tuyển dụng" của họ, nếu có nhu cầu thì bạn hãy nhanh chóng ứng tuyển bạn nhé!
          </p>
        </div>
  ...
  <div class="modal fade" id="Profile{{$key}}Modal" style="z-index: 9999">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Thông tin hồ sơ <span class="text-info">(lượt xem {{$profile->luotxem}})</span></h4>
